'use client'
import { getAllBookingsUser, getAllUser } from '@/backend/database';
import { ToogleAdminServerAction } from '@/backend/serverAction';
import { BookingsTypeDB, UserTypeDB } from '@/backend/Types';
...
export default function Table(){
    ...
    function fetchMore(){
        setLoading(true);
        getAllUser({start:range,limit:10}).then(e=>{
    ...
    return<>
         ...
         <div className="h-[88%] overflow-scroll scrollbar-hide rounded-lg">
          <table className="w-full text-sm  text-left rtl:text-right text-gray-500 dark:text-gray-400 relative rounded-xl">
              ...
              <tbody>
                  {data.length > 0 && <>
                      {data.map((e,i)=><TableRow data={e} index={i} key={i}/>)}
...
function TableRow({data,Actions,index}:{index:number,data:UserTypeDB,Actions?:()=>React.Component}) {
    const [preState,action,isPending] = useActionState(ToogleAdminServerAction,{error:"",success:false})
    ...
    function handleToogle(){
        ...
        formData.set("admin",`${data.admin ? "false" : "true"}`)
        startTransition(()=>{
            action(formData);
        })
    }
    return (
        <>
            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td className="px-6 py-4">
                        <button onClick={handleToogle} disabled={isPending} className={`font-medium text-${admin ? "red" : "green"}-600 mr-2 cursor-pointer `} >Make {admin? "User" :"Admin"}</button>
                    </td> 
            </tr>
